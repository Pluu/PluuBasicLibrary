...
public class Logger {
	...
	private static final String STACK_PREFIX = "com.edl";

	private static LoggerConfig initLogConfig() {
		boolean showLogs = BuildConfig.SHOW_LOGS;

		int printLogLevel = Log.DEBUG;
		if(!showLogs && Helper.DeployPhase.current() == Helper.DeployPhase.Release ) {
		...
		builder = builder.setDefaultTag(Tag.DEFAULT).setStackPrefix(STACK_PREFIX).setPrintLoggerLevel(printLogLevel);
